...
public class AuditLogServiceTests : IClassFixture<DatabaseFixture>
{
    ...
    {
        await using var ctx = await _db.CreateContextAsync();
        ctx.Users.Add(new User { UserId = 99, Username = "test", FullName = "Test", PasswordHash = "hash", RoleId = 1, IsActive = true, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow });
    ...
    {
        await using var ctx = await _db.CreateContextAsync();
        ctx.Users.Add(new User { UserId = 50, Username = "active1", FullName = "Active One", PasswordHash = "hash", RoleId = 1, IsActive = true, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow });
        ctx.Users.Add(new User { UserId = 51, Username = "inactive1", FullName = "Inactive One", PasswordHash = "hash", RoleId = 1, IsActive = false, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow });
        await ctx.SaveChangesAsync();
